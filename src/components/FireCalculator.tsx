      ...
      styleOverrides: {
        root: {
          '& .MuiTableCell-root': {
            ...
            color: '#000000',
            fontWeight: 600,
            borderBottom: '2px solid rgba(255, 69, 0, 0.2)',
            fontSize: '0.9rem',
          },
        },
      },
...
const darkFireTheme = createTheme({
  palette: {
    mode: 'dark',
    primary: {
      main: '#ff4500',
...
const FireCalculator: React.FC = () => {
  ...
  const getRowBackgroundColor = (savings: number, netWorthGrowth: number, netWorth: number) => {
    if (isDarkMode) {
      if (netWorth < 0) return 'rgba(255, 0, 0, 0.15)';  // Red for negative net worth
      if (netWorthGrowth < 0) return 'rgba(158, 113, 46, 0.31)';  // Orange for negative growth
      if (savings < 0) return 'rgba(156, 139, 42, 0.31)';  // Yellow for negative savings
      if (savings > 0) return 'rgba(0, 255, 0, 0.05)';  // Very subtle green for positive savings
      return '#1e1e1e';  // Default dark background
    } else {
      if (netWorth < 0) return '#ffebee';  // Light red for negative net worth
      if (netWorthGrowth < 0) return '#fff3e0';  // Light orange for negative growth
      if (savings < 0) return '#faf9c4';  // Light yellow for negative savings
      if (savings > 0) return '#e8f5e8';  // Light green for positive savings
      return '#ffffff';  // Default light background
    }
  };

  return (
    <ThemeProvider theme={isDarkMode ? darkFireTheme : lightTheme}>
      <CssBaseline />
      <Container 
        disableGutters
        sx={{ 
        ...
        <Box sx={{ mb: 4, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <Box>
            <Typography variant="h3" component="h1" gutterBottom sx={{ 
              ...
              <Box component="span" className="fire-text">
                FIRE
              </Box>
              <Box component="span" sx={{ 
                background: 'linear-gradient(45deg, #ff4c25, #8c5efd)',
                              ...
                              >
                                ...
                                >
                                  ...
                                  {results.fireAge === year && (
                                    <Box component="span" sx={{ ml: 1, color: '#ff4500', fontWeight: 'bold' }}>
                                      🔥 FIRE
                                    </Box>
                                  )}
                                </TableCell>
                                ...
                                <TableCell 
                                  align="right"
                                  sx={{ 
                                    color: results.realNetWorth[index] < 0 ? '#ff4500' : 'inherit',
